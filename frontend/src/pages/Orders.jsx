      ...
      >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50/80">
            ...
            <div className="w-1/5 text-center hidden sm:block">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Status</span>
            </div>
            <div className="w-1/5 text-center hidden md:block">
            ...
            <div>
              {filtered.map((order, i) => (
                <motion.div
                  ...
                  className="flex items-center justify-between px-5 py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50/70 transition-colors group"
                >
                  {/* Order & Customer */}
                  <div className="flex items-center gap-3 w-2/5 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 flex-shrink-0 font-bold text-sm">
                      #{order.id}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[13px] font-semibold text-slate-800 truncate">{getCustomerName(order.customer_id)}</p>
                      <p className="text-[11px] text-slate-400 mt-0.5 truncate">{getCustomerEmail(order.customer_id) || `ID: ${order.customer_id}`}</p>
                    </div>
                  </div>
                  ...
                  {/* Items & Total */}
                  <div className="w-1/5 text-center hidden md:flex flex-col items-center">
                  ...
                  {/* Actions */}
                  <div className="w-1/5 flex items-center justify-end gap-1.5 flex-shrink-0">
                    <Link
                      to={`/orders/${order.id}`}
                      className="h-8 px-3 rounded-lg flex items-center justify-center text-[12px] font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-transparent transition-all"
                    >
                      View Details
                    </Link>
                    <button
          ...
          {orderStatus !== 'loading' && filtered.length === 0 && (
            <div className="py-20 flex flex-col items-center gap-3 text-center">
              ...
              <p className="text-[13px] text-slate-400">Try a different search or create a new order.</p>
            </div>
          )}
        </div>
      </motion.div>
